 ...
 * @var string $activeNav index|filter|about|magnitu|researcher|label|feeds|scraper|mail|lex|leg|settings|configuration|styleguide|logbook
            ...
            <div class="top-bar-actions">
                ...
                <?php if (!empty($showTimelineRefresh) && ($activeNav === 'index' || $activeNav === 'filter')): ?>
                    <form id="seismo-timeline-refresh-form" method="post" action="<?= e($basePath) ?>/index.php?action=<?= e($timelineRefreshAct) ?>" class="admin-inline-form top-bar-form-gap seismo-ajax-refresh-form">
                        ...
                        <button type="submit" class="top-bar-btn top-bar-btn--text top-bar-btn--timeline-refresh" data-refresh-label="Refresh" title="<?= isSatellite() ? 'Triggers mothership refresh (feeds, press, scrapers, mail, Leg — Lex omitted, same as mothership toolbar), then recipe scoring on this desk' : 'Refresh feeds, press, scrapers, parliament calendar, and mail when due (mail/Gmail at most every 15 minutes). Lex legislation uses Diagnostics or cron.' ?>">Refresh</button>
                    </form>
                <?php endif; ?>
                <?php
