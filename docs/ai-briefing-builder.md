...
| Gemini pass-2 output | Scales **~4500 visible tokens per cited item** (floor 2048, practical cap **49152**; **one item per batched call** uses the full practical cap). Override via `gemini:max_output_tokens` in `system_config` (recommend **65536** for heavy Legal templates). **≥3 items:** pass 2 is batched upfront (2 keys/call, then 1/call on truncation). |
