...
| Gemini pass-2 output | Scales **~4500 visible tokens per cited item** (floor 2048, practical cap **49152**). Override via `gemini:max_output_tokens` in `system_config` (recommend **65536** for heavy Legal templates). **One** pass-2 call by default; batched retry (2 keys/call, then 1/call on further truncation) only when the monolithic response hits output limits. |
