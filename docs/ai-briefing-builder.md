...
| Gemini two-pass | **Skinny two-pass by default** (UI checkbox on): pass 1 = one **global** selection call (**USER PROMPT** + full entry bodies up to **2000** chars, JSON `used_entry_keys` only); pass 2 = **plain Markdown** on selected rows only (same USER PROMPT). Auto-enabled when pool ≥ **1** entry. Legacy single-pass if two-pass unchecked. |
