...
2. **Blick (Ringier)** — section RSS, e.g. `https://www.blick.ch/politik/rss.xml`. **Extract full text on** (full HTML, not paywalled like TA). RSS and article fetch both need **`seismoHttpUserAgent()`** (with `(+url)` contact); Ringier returns **403** for bare `Seismo/x.y` and for the Scraper Chrome UA. Set `SEISMO_MOTHERSHIP_URL` in `config.local.php` on the VPS if needed.
